...
Major rework of the validation pipeline. The project settles on a single, clean
reconstruction using exclusively `GLOBAL_MULTIYEAR_BGC_001_029` as the pCO₂
source. The MULTIOBS pCO₂ reconstruction is dropped. Validation is substantially
expanded: Fay (2014) domain analysis, LOESS multi-year variability, power spectra,
and cross-power spectrum replace the Taylor diagram.
...
#### `scripts/03_compute_flux.py`
- Removed all code related to the MULTIOBS pCO₂ improved reconstruction.
- Single reconstruction only: `GLOBAL_MULTIYEAR_BGC_001_029` pCO₂ + Wanninkhof
  (2014) k (with wind variance correction when ERA5 daily available) + Weiss (1974)
  K0 from GLORYS12 SST/SSS.
- Output files simplified: `flux_3d.nc` contains `fgco2`, `k`, `K0`, `Sc` only;
  `global_flux.nc` contains `J_net_PgC` and `J_net_PgC_annual` only.
- Product label in NetCDF attributes updated to `GLOBAL_MULTIYEAR_BGC_001_029`.

#### `scripts/04_validate.py` — full rewrite
- **Sign fix**: `fgco2_obs` loaded without negation (`fgco2_obs = ds_surf["fgco2_obs"]`).
  Previous versions applied `-` which may have been incorrect depending on the
  MULTIOBS product version.
- **Dropped**: Taylor diagram, improved reconstruction curve, 2×2 map layout.
- **Dropped**: MULTIOBS pCO₂ reconstruction (green curve).
- **fig_validation_ts.png**: two curves only — `GLOBAL_MULTIYEAR_BGC_001_029`
  reconstruction (blue) vs MULTIOBS SOCAT-NN (red dashed).
- **fig_validation_loess.png** [NEW]: raw monthly + LOESS smooth (frac=0.15,
  ≈18-month window) overlaid for both curves, revealing multi-year variability
  without spectral analysis.
- **fig_validation_map.png**: simplified to 1×2 (RMSD top, Bias bottom), fixed
  colourbar extents, contourf, Robinson projection. Product labels updated.
- **fig_fay_domains.png** [NEW]: 17-panel figure (one per Fay 2014 biome) +
  global biome map in panel 18. Each panel shows full monthly time series on the
  left y-axis and climatological seasonal cycle on the right y-axis, for both
  reconstruction and MULTIOBS. Inset domain map per panel. Loads biome mask from
  `data/Time_Varying_Biomes.cmems.nc` (generated by `interpolator.py`).
- **fig_spectra.png** [NEW]: log-log PSD (Welch's method) of global integral —
  reconstruction vs MULTIOBS overlaid; x-axis in years (period); −5/3 reference
  slope annotated.
- **fig_cps.png** [NEW]: cross-power spectrum with MULTIOBS as reference. Three
  panels: gain, phase [days], squared coherence. High nfft (next power of 2 × 4)
  for smooth output. Annual and semi-annual periods marked.
- **validation_metrics.csv**: now includes both global scalar metrics and
  per-domain metrics for all 17 Fay biomes.
- All figure labels use `GLOBAL_MULTIYEAR_BGC_001_029` instead of "PISCES".
- Global plot style set via `rcParams` for consistent paper-grade appearance.

### Dependencies added
- `statsmodels` — for LOWESS smoother (`statsmodels.nonparametric.smoothers_lowess`)

### Data requirements added
- `data/Time_Varying_Biomes.cmems.nc` — Fay (2014) biome mask on CMEMS 0.25°
  grid, generated by running `interpolator.py cmems` in the biomes directory.

---

[1.2.0]: https://github.com/[your-repo]/releases/tag/v1.2.0
[1.1.2]: https://github.com/[your-repo]/releases/tag/v1.1.2
[1.1.1]: https://github.com/[your-repo]/releases/tag/v1.1.1
[1.1.0]: https://github.com/[your-repo]/releases/tag/v1.1.0
[1.0.0]: https://github.com/[your-repo]/releases/tag/v1.0.0
